...
typedef enum {            
    ...
    MAP_TYPE_TILE = 2        // Interior tile-based multi-nivel  
} MAP_TYPE;
         
typedef struct {                  
    int64_t id;            
    MAP_TYPE type;  // MAP_TYPE_TILE para el nuevo sistema  
    ...
    // Datos para modo tile multi-nivel (interiores DMAP)  
    TILE_CELL *tile_grid;       // Grid 3D: [level][y][x]  
    uint32_t grid_width;  
    uint32_t grid_height;  
    uint32_t num_levels;  
    float tile_size;  
      
    THIN_WALL *thin_walls;  
...
typedef struct {          
    ...
    float fov;          
    float near, far;  
    float plane_x, plane_y;  // AÑADIR ESTOS CAMPOS  
} CAMERA_3D;      
...
// Funciones para mapas DMAP tile-based  
